...
user.mab = function(u, niterations){
  ...
  # UCB1 as seen in https://rpubs.com/OttoP/478713  
  ucb1 <- function(actions, rewards, episode)
  {
      successrates <- (0.5+rewards)/(1+actions)
      confbound <- as.data.frame(lapply(actions, function(arow) { return (sqrt(2*log(episode+1)/(arow+1e-10))) }))
  
      a <- apply(successrates+confbound, 1, which.max)
      return(a)
  }

  # Thompson Sampling as seen in https://rpubs.com/OttoP/478713  
  ...
  for(iter in 1:niterations) {
    ...
    scalarized.rewards = scalarize(mean.reward.affinity, mean.reward.diversity)
    for(i in 1:REC.SIZE){
      
      rec.list[i] = thompson_sampling(ni[i,], scalarized.rewards[i], iter)
      #rewards[, rec.list[i]] = 0
    ...
    fwrite(df,
           paste0(address, "resultsmomabucb1/rewards.user", u, ".txt"),
