...
calculate.daily.regret = function(day.index, mean.regret){
...
generate.regret.graphics = function(user.result){
  ...
  for(day in 1:nrow(regrets)){
    #if (day <= NDAYS){
        # mean.regret = c(mean.regret, sum(regrets$regret[1:day]) / day) # regrets mean 
    ...
    #} else {
    #    mean.regret = c(mean.regret, sum(regrets$regret[(day-NDAYS):day]) / NDAYS) # consider only the last days
    #}
  }
