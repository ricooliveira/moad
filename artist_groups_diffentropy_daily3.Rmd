...
user.genre.daily = function(user){
  ...
  x = apply(wmd.genre, 1, sort, decreasing = T) %>% t() %>% as.data.frame()
...
nm <- list.files(path=paste0(address, "data/wmd"))
nm = nm[1:990]
users = substr(nm,25,nchar(nm) - 4)

#for(user in users){
user.propensity.diffentropy = function(user){
  wmd.contemporaneity = fread(paste0(address, "data/wmd/wmd.contemporaneity.user", user, ".txt"),
                            sep = "|", 
                            na.strings = "")

  wmd.locality = fread(paste0(address, "data/wmd/wmd.locality.user", user, ".txt"),
                            sep = "|", 
                            na.strings = "")

  wmd.genre = fread(paste0(address, "data/wmd/wmd.genre.user", user, ".txt2"),
                            sep = "|", 
                            na.strings = "")
  ...
  LFM.compact = LFM %>% select(c(userid, artistname, timestamp)) %>% filter(user == userid)
  days = unique(LFM.compact$timestamp)
  propensity.diff.entropy = cbind(days, propensity.diff.entropy)
...
pblapply(users, user.propensity.diffentropy)
```
