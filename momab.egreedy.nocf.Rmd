...
multi.objective = function(list.elements.index, artist.data, data.train.user, aspects.to.diversify){
...
user.mab = function(u, iterations, EPSILON){
  ...
  for(i in 1:(NARTISTS / REC.SIZE)){
    ...
    reward = multi.objective(rec.list, artist.data, data.train.user, aspects.to.diversify)
  ...
  for(iter in 1:iterations) {
    ...
    for(i in 1:REC.SIZE){
      ...
      drawn = runif(1, 0, NARTISTS)
      if(drawn > EPSILON * NARTISTS){
    ...
    fwrite(df,
           paste0(address, "resultsmomabgreedynocf/rewards.user", u, ".egreedy.200iter.txt"),
...
a = Sys.time()
#user.mab(7687, 1000, EPSILON)
pblapply(users[17:997], user.mab, 200, EPSILON)
print(Sys.time() - a)
