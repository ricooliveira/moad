...
# daily ILDs generation - do not run - already generated
...
sd.contemporaneity = sd(all.ilds.filtered$contemporaneity)
sd.locality = all.ilds.filtered$locality
sd.genre = all.ilds.filtered$genre

# adding distribution graphics

contemporaneity.ild.distribution = ggplot(all.ilds.filtered, aes(x = contemporaneity)) + geom_density()
locality.ild.distribution = ggplot(all.ilds.filtered, aes(x = locality)) + geom_density()
genre.ild.distribution = ggplot(all.ilds.filtered, aes(x = genre)) + geom_density()

```

```{r}

# daily graphics - do not run
...
user.propensity = function(user, history){
  print(user)
  ...
  fwrite(c(user, user.ilds) %>% t() %>% as.data.frame(),
        paste0(address,"resultsmomab/allhistory.ILD.allusers.txt"), 
...
lapply(users, user.propensity, data.train)
