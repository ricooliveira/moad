...
# Multiobjective Ranked MAB Thompson Sampling
...
# Thompson sampling implemented as seen in
...
#REC.SIZE = 5
#NARTISTS = 10
user.mab = function(u, iterations){
  ...
  reward.affinity = rep(0.0, NARTISTS)
  reward.diversity = rep(0.0, NARTISTS)
  penalties.affinity = rep(0.0, NARTISTS)
  penalties.diversity = rep(0.0, NARTISTS)
  
  reward.affinity = reward.affinity %*% t.default(rep(0,REC.SIZE))
  reward.diversity = reward.diversity %*% t.default(rep(0,REC.SIZE))
  penalties.affinity = penalties.affinity %*% t.default(rep(0,REC.SIZE))
  penalties.diversity = penalties.diversity %*% t.default(rep(0,REC.SIZE))
  
  total.reward.affinity = rep(0.0, NARTISTS)
  total.reward.diversity = rep(0.0, NARTISTS)
  
  # generate recommendation list
  # test
  # iter = 1
  # a = Sys.time()
  quantiles <- seq(0, 1, length = NARTISTS + 1)
  #beta.d.diversity = list
  for(iter in 1:iterations) {
    # iter=1
    for (bandit in 1:REC.SIZE) {
      # bandit=1
      # apply(c(1:NARTISTS), dbeta, quantiles, reward.affinity[bandit, arm] + 1, penalties.affinity[bandit, arm])
      for(arm in 1:NARTISTS){
        # arm=1
        # beta_d = random.betavariate(rewards[i] + 1, penalties[i] + 1)
        beta.d = dbeta(quantiles, reward.affinity[bandit, arm] + 1, penalties.affinity[bandit, arm])
      }
    }
