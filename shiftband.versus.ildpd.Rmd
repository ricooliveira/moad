...
baseline.regrets = function(user, days){
  ...
  freeze.days = floor(nrow(scenarios.ildpd) / 4 + 1)
  new.columns = data.frame()
  while(freeze.days < nrow(scenarios.ildpd)){
    new.columns = rbind(cbind(rep(scenarios.ildpd[freeze.days,2], days), rep(scenarios.ildpd[freeze.days,3], days), rep(scenarios.ildpd[freeze.days,4], days)))
    freeze.days = freeze.days + days
  }
  #for(i in freeze.days:nrow(scenarios.ildpd))
