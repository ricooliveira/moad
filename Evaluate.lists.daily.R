...
# u = 7687
affinity.by.user.daily = function(u, data){
  data.user = filter(data, users == u)
  
}

colnames(data.test)[3] = "users"
colnames(data.test)[10] = "artistname"
data.test$timestamp = (data.test$timestamp / (60*60*24)) %>% floor() # timestamp em segundos transformado pra dias
data.test.grouped = select(data.test, "users", "timestamp", "artistname")
data.test.grouped = data.test.grouped[order(data.test.grouped$users, data.test.grouped$timestamp),]

# x = data.test.grouped %>% group_by(users, timestamp)
...
affinity = function(day, history.user){
  history.user.day = filter(history.user, timestamp == day)
  history.user.artists = history.user.day$artistname %>% unique()
  history = artist.data[which(artist.data$Artist %in% history.user.artists),]
  sum.dlh = 0
  for(i in aspects.not.to.diversify){
    df.aspect = df[(aspects[[i]])]
    sum.dlh = sum.dlh + DLH(data = df.aspect, history = history.user.day[(aspects[[i]])], distance.function = distance.functions[[i]])
  }
  y2 = (sum.dlh / length(aspects.not.to.diversify))
}


multi.objective = function(list.elements.index, data.test.user){
  ...
  days = data.test.user$timestamp %>% unique()
  y2 = lapply(days, affinity, data.test.user) %>% as.data.frame() %>% mean()
...
obj.scen1 = fread("sample1000.nsga.fixed.top10.pop10.gen20.scenario1.txt", sep = ",", na.strings = "", header = F)
obj.scen2 = fread("sample1000.nsga.fixed.top10.pop10.gen20.scenario2.txt", sep = ",", na.strings = "", header = F)
obj.scen3 = fread("sample1000.nsga.fixed.top10.pop10.gen20.scenario3.txt", sep = ",", na.strings = "", header = F)
obj.scen4 = fread("sample1000.nsga.fixed.top10.pop10.gen20.scenario4.txt", sep = ",", na.strings = "", header = F)
obj.scen5 = fread("sample1000.nsga.fixed.top10.pop10.gen20.scenario5.txt", sep = ",", na.strings = "", header = F)
obj.scen6 = fread("sample1000.nsga.fixed.top10.pop10.gen20.scenario6.txt", sep = ",", na.strings = "", header = F)
obj.scen7 = fread("sample1000.nsga.fixed.top10.pop10.gen20.scenario7.txt", sep = ",", na.strings = "", header = F)
obj.scen8 = fread("sample1000.nsga.fixed.top10.pop10.gen20.scenario8.txt", sep = ",", na.strings = "", header = F)
...
colnames(objectives) = c("users", "artistname", "scenario")

archive.name = "user.propensity.entropy.daily.txt"
user.propensity.w2v = fread(paste0("/local/Scripts/word2vec/MOAD/data/", archive.name), sep = "\t", na.strings = "")
...
user.propensity.w2v$users = as.character(user.propensity.w2v$users)

results = inner_join(user.propensity.w2v, objectives, by = c("users", "scenario")) %>% select(users, artistname, scenario)
...
for(u in users){
  ...
  scenario = results %>% filter(users == u) %>% select(scenario) %>% unique() %>% as.numeric()
  # Vou apelar
  if(scenario == 0){
    aspects.to.diversify = NULL
  } else if(scenario == 1) {
    aspects.to.diversify = 3
  } else if(scenario == 2) {
    aspects.to.diversify = 2
  } else if(scenario == 3) {
    aspects.to.diversify = c(2,3)
  } else if(scenario == 4) {
    aspects.to.diversify = 1
  } else if(scenario == 5) {
    aspects.to.diversify = c(1,3)
  } else if(scenario == 6) {
    aspects.to.diversify = c(1,2)
  } else if(scenario == 7) {
  ...
  data.test.user = filter(data.test, users == u) 
  df = multi.objective(results[results$user == u,]$position, data.test.user)
  df.objectives[user,1] = u; df.objectives[user,2] = df[1]; df.objectives[user,3] = df[2]; df.objectives[user,4] = scenario
}
