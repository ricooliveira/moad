...
```{r}
  ...
  user.2.dtm <- function(userday, data){
    vec = filter(data, userid == as.numeric(userday[1]) & timestamp == as.numeric(userday[2])) %>% select(artistname) %>% as.vector() %>% unique()
    return(format_vectors(vec, paste0(userday[1], "-", userday[2])))
  }
...
userdays = select(LFM, c(userid, timestamp)) %>% unique()
#userdays = userdays[1:100,]
LFM.compact = LFM %>% select(c(userid, artistname, timestamp))
user.formatted.vectors = do.call(rbind, pbapply(userdays, 1, user.2.dtm, LFM.compact))
...
targets=dtm[1:(userdays %>% nrow()),]
features=dtm[(userdays %>% nrow()+1):(genres %>% length()+userdays %>% nrow()),]
...
fwrite(wmd.genre,
       "data/wmd.genre.daily.txt",
       sep = "\t",
       row.names = F, col.names = F)

fwrite(userdays,
       "data/wmd.users.daily.txt")
...
targets=dtm[1:(userdays %>% nrow()),]
features=dtm[(userdays %>% nrow()+1):(localities %>% length()+userdays %>% nrow()),]
...
targets=dtm[1:(userdays %>% nrow()),]
features=dtm[(userdays %>% nrow()+1):(decades %>% length()+userdays %>% nrow()),]
...
fwrite(wmd.contemporaneity %>% as.list(),
       "data/wmd.contemporaneity.daily.txt",
       sep = "\t",
       row.names = F, col.names = F)
...
wmd.contemporaneity = fread("data/wmd.contemporaneity.daily.txt",
                            sep = "\t", 
                            na.strings = "")

wmd.locality = fread("data/wmd.locality.daily.txt",
                            sep = "\t", 
                            na.strings = "")

wmd.genre = fread("data/wmd.genre.daily.txt",
                            sep = "\t", 
                            na.strings = "")
...
diff.entropy.contemporaneity = pbapply(wmd.contemporaneity %>% as.matrix(), 1, differential.entropy)
diff.entropy.locality = pbapply(wmd.localities %>% as.matrix(), 1, differential.entropy)
diff.entropy.genre = pbapply(wmd.genre %>% as.matrix(), 1, differential.entropy)
...
propensity.diff.entropy = cbind(userdays, 
...
propensity.diff.entropy = cenarios.decimal(propensity.diff.entropy)

users = userdays$userid %>% unique()

scenario.moda.user = function(u, data){
  all.scenarios.user = filter(data, userid == u) %>% select(scenario)
  return(moda(all.scenarios.user))
}

propensity.diff.entropy.daily = data.frame(users, scenarios = lapply(users, scenario.moda.user, propensity.diff.entropy) %>% as.data.frame() %>% t())

fwrite(propensity.diff.entropy.daily,
       "data/user.propensity.word2vec.diffentropy.daily.gaussian.txt",
       sep = "\t",
       row.names = F, col.names = F)
```
