...
coincidence = function(real, chosen){
  if(length(chosen) == length(real)){
    ...
    sum.vec = ((sum.vec %% 2) - 1) %>% abs() %>% ceiling()
...
calculate.w2v.regrets = function(user){
  
  w2v = fread(paste0(address, "resultsw2v/user.daily.propensity.word2vec.diffentropy.gaussian.user", user,".txt"),
        sep = "\t", header = F)
  days = w2v %>% select(V1)
  w2v = w2v %>% select(V2, V3, V4)
  ilds = fread(paste0(address, "results/daily.propensity.scenario.user", user,".txt"),
        sep = "\t", header = F)
  ilds = ilds %>% select(V2,V3,V4)
  
  linha = data.frame(V2 = sample(c(0,1), size = 1), V3 = sample(c(0,1), size = 1), V4 = sample(c(0,1), size = 1))
  w2v = bind_rows(linha, w2v) %>% slice(-n())
  w2v = cbind(days, w2v, ilds)

  day.regret = function(vec){
    return(1 - coincidence(vec[2:4], vec[5:7]))
  }
  
  regrets = cbind(days, apply(w2v, 1, day.regret)) %>% as.data.frame()
  names(regrets) = c("day", "regret")
  
  fwrite(regrets,
         paste0(address,"resultsw2vregrets/w2v.daily.regret.nomean.user", user, ".txt"),
         row.names = FALSE, col.names = FALSE, sep = "\t")
   
}

# for incomplete results
nm = list.files(path = paste0(address, "resultsw2v"))
users.w2v = substring(nm, 57, nchar(nm) - 4)

pblapply(users.w2v, calculate.w2v.regrets)
```

```{r}

# calculate line areas
...
generate.line.areas = function(user){
  ...
  if(sb %>% nrow() > 1){
    ...
    w2v = fread(paste0(address, "resultsw2vregrets/w2v.daily.regret.nomean.user", user, ".txt"),
                sep = "\t", header = F)
    
    regrets = cbind(sb, etp, ildpd, w2v)  
    regrets = regrets[,c(-1, -3, -5, -7)]
    names(regrets) = c("ShiftBand", "Entropy", "ILDPD", "Word2vec")
    
    apply(regrets, 2, integrate.regrets) %>% as.data.frame()
    
  }
}

regret.area = do.call(rbind, pbsapply(users.w2v, generate.line.areas))

fwrite(regret.area %>% as.data.frame(), 
       paste0(address, "resultsarea/regret.area.sb.entropy.ildpd.w2v.nomean.testset20pct.txt"),
       sep = "\t",
       row.names = F,
       col.names = T)
