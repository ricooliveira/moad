...
#NARTISTS = 10
#niterations = 500
user.mab = function(u, niterations){
  ...
  #   reward = multi.objective(rec.list, artist.data)
  ...
  reward.affinity = mean.reward.affinity
  reward.diversity = mean.reward.diversity
  
  ni = matrix(1, REC.SIZE, NARTISTS)
  
  random <- function(actions) 
  {
    a <- runif(1, max = length(actions)) %>% trunc()
    return(a)
  }
  
  greedy_tie_breaker <- function(actions, rewards) 
  {
    successrates <- (0.5+rewards)/(1+actions)
    a <- successrates %>% which.is.max() # which.is.max randomizes between ties
    return(a)
  }
  
  epsilon_greedy <- function(actions, rewards, epsilon = 0.1) 
  {
    r <- random(actions)
  ...
  # UCB1 in https://rpubs.com/OttoP/478713
  ...
  # Thompson sampling in https://rpubs.com/OttoP/478713
  thompson_sampling <- function(actions, rewards) 
  {
      ...
      probs <- sapply(seq(length(actions)), function(j) {return(rbeta(1, rewards[j]+1, (actions[j]-rewards[j])+1))})
  ...
  for(iter in 1:niterations) {
    # i=1
    reward.scalarized = scalarize(reward.affinity, reward.diversity)
    rec.list = rep(0,REC.SIZE)
    
    for(i in 1:REC.SIZE){
      rec.list[i] = epsilon_greedy(ni[i,], reward.scalarized[i,])
    ...
    fwrite(df,
           paste0(address, "resultsmomabts/rewards.user", u, ".iter", niterations, ".egreedy.txt"),
    ...
    # i = 1
    for(i in 1:REC.SIZE){
      mean.reward.diversity[i,rec.list[i]] = ((mean.reward.diversity[i,rec.list[i]] * (ni[i,rec.list[i]]) + reward[1]) / (ni[i,rec.list[i]] + 1)) 
      mean.reward.affinity[i,rec.list[i]] = ((mean.reward.affinity[i,rec.list[i]] * (ni[i,rec.list[i]]) + reward[2]) / (ni[i,rec.list[i]] + 1)) 
      if(reward.scalarized[i, rec.list[i]] < scalarize(reward[1], reward[2])){  # problem
        reward.diversity[i, rec.list[i]] = reward[1] 
        reward.affinity[i, rec.list[i]] = reward[2] 
      }
    }
  }
  # print(Sys.time() - a)
