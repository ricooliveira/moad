  ...
  for(iter in 1:NITER) {
    ...
    for(i in 1:REC.SIZE){
      if(ni[i, rec.list[i]] == 1){
        ...
        user.arms.diversity[i,rec.list[i]] = ((user.arms.diversity[i,rec.list[i]] * ni[i,rec.list[i]] + reward[1]) / (ni[i,rec.list[i]] + 1)) + sqrt(2 * log(iter) / ni[i,rec.list[i]])
      }
      user.arms.affinity[i,rec.list[i]] = ((user.arms.affinity[i,rec.list[i]] * ni[i,rec.list[i]] + reward[2]) / (ni[i,rec.list[i]] + 1)) + sqrt(2 * log(iter) / ni[i,rec.list[i]])
    }
  }
  # print(Sys.time() - a)
}
