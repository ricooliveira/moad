...
user.mab = function(u, niterations){
  ...
  user.arms.affinity = rep(1.0, NARTISTS)
  user.arms.diversity = rep(1.0, NARTISTS)
  ...
  for(i in 1:(NARTISTS / REC.SIZE)){
    ...
    user.arms.diversity[rec.list] = reward[1]
    user.arms.affinity[rec.list] = reward[2]
  }
  #print(Sys.time() - a)
  
  user.arms.affinity = rep(1, REC.SIZE) %*% t.default(user.arms.affinity)
  user.arms.diversity = rep(1, REC.SIZE) %*% t.default(user.arms.diversity)
  
  ni = rep(1, NARTISTS)
  ni = rep(1, REC.SIZE) %*% t.default(ni)
  ...
  for(iter in 1:niterations) {
    ...
    for(i in 1:REC.SIZE){
      scalarized.objectives = scalarize(user.arms.affinity[i,], user.arms.diversity[i,])
      rec.list[i] = which.max(scalarized.objectives)
    ...
    reward = multi.objective(rec.list, artist.data)
    fwrite(reward %>% as.data.frame(),
           paste0(address, "resultsmomab/rewards.user", u, ".txt"),
    ...
    # i = 1
    for(i in 1:REC.SIZE){
    #  if(ni[i, rec.list[i]] == 1){
    #    user.arms.diversity[i,rec.list[i]] = reward[1]
    #  } else {
      # FIXME
      user.arms.diversity[i,rec.list[i]] = ((user.arms.diversity[i,rec.list[i]] * (ni[i,rec.list[i]]-1) + reward[1]) / (ni[i,rec.list[i]])) + sqrt(2 * log(iter) / ni[i,rec.list[i]])
    # }
      user.arms.affinity[i,rec.list[i]] = ((user.arms.affinity[i,rec.list[i]] * (ni[i,rec.list[i]]-1) + reward[2]) / (ni[i,rec.list[i]])) + sqrt(2 * log(iter) / ni[i,rec.list[i]])
    }
  }
  # print(Sys.time() - a)
}
