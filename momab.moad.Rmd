...
GAMMA = 0.1
ETA = 0.1
K = nrow(artist.data) # Number of artists
TRIALS = 10 
S = 200000
...
names(propensities) = c("user", "div.contemporaneity", "div.locality", "div.genre")

fwrite(rbind(user, propensities),
...
one.user.momab.shiftband = function(user){
  ...
  for (i in 1:S) { # each user iteraction
    ...
    user.prop = user.prop %>% filter(user.prop$user == user) %>% select(div.contemporaneity, div.locality, div.genre)
    aspects.to.diversify = which(user.prop)
    aspects.not.to.diversify = setdiff(aspects.all,aspects.to.diversify)
    
    reward = multi.objective(rec.list)
    
    # set xit
    xit.diversity = matrix(data = 0, nrow = REC.SIZE, ncol = K)
    xit.affinity = matrix(data = 0, nrow = REC.SIZE, ncol = K)
    
    #xit = xit.diversity
    #p = p.diversity
    for(pos in 1:REC.SIZE){
      xit.diversity[pos, rec.list[pos]] = reward[1] / p[pos, rec.list[pos]]
      xit.affinity[pos, rec.list[pos]] = reward[2] / p[pos, rec.list[pos]]
    }
