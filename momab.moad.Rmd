...
for(i in 1:length(users)){
  ...
  fwrite(df.user, 
         paste0(address,"resultsmomab/sample1000.ubcf.top1000.csv"),
...
# AUER, Peter. Using confidence bounds for exploitation-exploration trade-offs. Journal of Machine Learning Research, v. 3, n. Nov, p. 397-422, 2002.
  ...
  set.weights = function(i, w, p, x.exp, W){
    return(w[i] * exp(ETA * (x.exp[i] + ALPHA / (p[i] * sqrt(TRIALS * K / S)))) + (BETA/K) * W) # S = number of days)
  }
...
ubcf <- fread(paste0(address,"resultsmomab/sample1000.ubcf.top1000.csv"), 
                    sep = ";", 
                    verbose = FALSE,
                    header = FALSE)
ubcf = as.data.frame(ubcf)
names(ubcf) = c("userid", "Artist")
# user = users[1]
one.user.momab.shiftband = function(user, ubcf){
  ...
  # filter ubcf list for user
  ubcf.user = ubcf %>% filter(userid == user)
  ubcf.user.artistdata = artist.data %>% filter(artist.data$Artist %in% ubcf.user$Artist)
  ...
  for (i in 1:TRIALS) { # each user iteraction
    ...
    reward = multi.objective(rec.list,ubcf.user.artistdata)
    ...
    for(pos in 1:REC.SIZE){
      w.matrix.diversity[pos, ] = lapply(c(1:K), set.weights, w.matrix.diversity[pos,], 
                                       p.diversity[pos,], xit.diversity[pos,], W.diversity[pos]) %>% as.numeric()
      w.matrix.affinity[pos, ] = lapply(c(1:K), set.weights, w.matrix.affinity[pos,], 
                                       p.affinity[pos,], xit.affinity[pos,], W.affinity[pos]) %>% as.numeric()
    }
    # w.matrix.diversity[, 1:15]
...
one.user.momab.shiftband(user, ubcf)
